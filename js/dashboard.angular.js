		...
		$scope.items_total = function() {
			...
			for( i = 0; i < $scope.orders.length; i++ ) {
				for( j = 0; j < $scope.orders[i].items.length; j++ ) {
			 		if( typeof $scope.temp['id_'+$scope.orders[i].items[ j ].id] === 'undefined' ) {
			  			$scope.temp['id_'+$scope.orders[i].items[ j ].id] = {
			   				"name": $scope.orders[ i ].items[ j ].title,
			   				"quantity": 0
			  			};
			 		}
			 		$scope.temp['id_'+$scope.orders[i].items[ j ].id].quantity += $scope.orders[ i ].items[ j ].quantity;
				}
		   	}
		   	for (var k in $scope.temp) {
		   	    $scope.items.push($scope.temp[k]);
		   	}
		}
