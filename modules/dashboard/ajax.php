		...
		switch( $action ) {
			...
			case "add_expense":
				...
				if( !empty( $expense->details ) && !empty( $expense->amount ) ) {
					doquery("insert into expense(datetime_added, details, amount, payment, added_by) values('".date("Y-m-d H:i:s")."', '".slash($expense->details)."', '".slash($expense->amount)."', '".slash($expense->amount)."', '".$_SESSION["logged_in_admin"]["id"]."')", $dblink);
					$id = inserted_id();
				...
				if( isset( $order->items ) && is_array( $order->items ) ) {
					if( count( $order->items ) > 0 ) {
						if( isset( $order->id ) && is_numeric( $order->id ) ) {
							...
							doquery( "insert into sales( date ) values('".date("Y-m-d H:i:s")."')", $dblink );
